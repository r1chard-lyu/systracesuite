...
@mcp.tool
def bpftrace_help():
    """Gets the bpftrace help text."""
    try:
        result = subprocess.run(["bpftrace", "--help"], capture_output=True, text=True, check=True)
        return result.stdout
    except FileNotFoundError:
        return "Error: 'bpftrace' command not found. Please ensure bpftrace is installed and in your PATH."
    except subprocess.CalledProcessError as e:
        return f"Error executing bpftrace --help: {e.stderr}"
